...
## 2024-05-24 - Classification Preview Accessibility Fixes
**Learning:** Custom design system action buttons and form layouts often lack accessible label associations (htmlFor/id) and visible focus indicators, making them difficult for screen reader and keyboard users to navigate.
**Action:** Always verify that input fields have explicitly linked `<label>` tags, icon-only buttons have descriptive `aria-label`s, and custom interactive elements implement `focus-visible` states to ensure robust accessibility.
