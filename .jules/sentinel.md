...
## 2024-06-25 - Argument Injection Vulnerability via Unsanitized Paths in `subprocess.run`
**Vulnerability:** The codebase had multiple instances of argument injection vulnerabilities where dynamically constructed or system-provided paths were passed to `subprocess.run` (e.g., `subprocess.run(['du', '-sh', str(source_dir)])` or `subprocess.run(['file', '--mime-type', '-b', str(file_path)])`). An attacker who controls a filename or directory name could set it to start with a hyphen (`-`), causing the `du`, `file`, or `rm` command to misinterpret the file path as an unintended command-line option.
**Learning:** `subprocess.run` does not inherently protect against argument injection if an argument starts with a hyphen, as command-line tools often parse any such argument as an option, regardless of its position or intended meaning. Stringifying a `Path` object using `str(path)` retains relative or plain names (like `-rf` or `--delete`), triggering the vulnerability.
**Prevention:** 1. Always convert `pathlib.Path` objects to absolute strings using `str(path.absolute())` (or `str(path.resolve().absolute())`) when passing them to `subprocess.run`, ensuring the path starts with a directory separator (e.g., `/` on Unix) instead of a hyphen. 2. Explicitly validate that user-provided inputs do not start with a hyphen (e.g., `if path_str.startswith('-'): ...`) before processing them, as defense-in-depth. 3. When parsing file paths, strictly resolve them (`path.resolve(strict=True)`) to ensure the target exists and normalize it to block traversal payloads.
