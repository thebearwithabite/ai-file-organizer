...
class ContentExtractor:
    ...
    def __init__(self, base_dir: str = None):
        ...
        self.cache_dir = self.base_dir / "cache"  # Define cache dir
    ...
    def extract_content(self, file_path: Path, db_connection: Optional[sqlite3.Connection] = None) -> Dict[str, Any]:
        ...
        if self._is_content_cached(file_path, current_hash, db_connection):
            return self._get_cached_content(file_path, db_connection)
        ...
        # Cache the result
        self._cache_content(file_path, current_hash, result, db_connection)
    ...
    def _is_content_cached(self, file_path: Path, file_hash: str, db_connection: Optional[sqlite3.Connection] = None) -> bool:
        ...
        query = "SELECT file_hash FROM content_index WHERE file_path = ?"

        if db_connection:
            cursor = db_connection.execute(query, (str(file_path),))
            result = cursor.fetchone()
            return result and result[0] == file_hash
        else:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(query, (str(file_path),))
                result = cursor.fetchone()
                return result and result[0] == file_hash
    ...
    def _cache_content(self, file_path: Path, file_hash: str, result: Dict[str, Any], db_connection: Optional[sqlite3.Connection] = None):
        ...
        try:
            ...
            insert_fts_sql = """
                        ...
                        VALUES (?, ?, ?)
                    """
            insert_fts_params = (str(file_path), result['text'], self._safe_json_dumps(result['metadata']))

            if db_connection:
                # Use provided connection
                db_connection.execute(insert_main_sql, insert_main_params)

                if result['success'] and result['text']:
                    db_connection.execute(insert_fts_sql, insert_fts_params)

                # Commit to ensure persistence when using shared connection
                db_connection.commit()
            else:
                with sqlite3.connect(self.db_path) as conn:
                    # Store in main index
                    conn.execute(insert_main_sql, insert_main_params)

                    # Store in FTS index if extraction successful
                    if result['success'] and result['text']:
                        conn.execute(insert_fts_sql, insert_fts_params)
