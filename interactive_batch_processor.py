...
class InteractiveBatchProcessor:
    ...
    def start_batch_session(self, source_directory: str, session_name: str = None) -> str:
        ...
        try:
            with sqlite3.connect(self.batch_db_path) as conn:
                # NEW: Reuse ContentExtractor database connection as well
                with sqlite3.connect(self.content_extractor.db_path) as content_conn:
    ...
    def _generate_file_preview(self, file_path: Path,
                             db_connection: Optional[sqlite3.Connection] = None,
                             content_db_connection: Optional[sqlite3.Connection] = None) -> Optional[FilePreview]:
            ...
            try:
                if file_path.suffix.lower() in ['.txt', '.md', '.py', '.js', '.css', '.html']:
                    ...
                    extraction_result = self.content_extractor.extract_content(
                        file_path,
                        db_connection=content_db_connection
                    )
