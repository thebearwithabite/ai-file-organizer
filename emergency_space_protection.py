...
class EmergencySpaceProtection:
    ...
    def __init__(self, base_dir: str = None):
        ...
        try:
            ...
            # Try centralized path resolution first, then generic detection
            try:
                _root = get_ai_organizer_root()
                if _root and Path(_root).exists():
                    self.gdrive_root = Path(_root).parent  # My Drive level
            except Exception:
                pass
            ...
            if not self.gdrive_root:
                ...
                possible_gdrive_paths = (
                    glob.glob(str(Path.home() / "Library" / "CloudStorage" / "GoogleDrive-*" / "My Drive"))
                    + ["/Volumes/GoogleDrive/My Drive", "/Volumes/GoogleDrive"]
                )
                for p in possible_gdrive_paths:
                    if Path(p).exists() and Path(p).is_dir():
                        self.gdrive_root = Path(p)
                        break

            if self.gdrive_root and getattr(self, 'logger', None):
