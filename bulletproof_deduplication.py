...
class BulletproofDeduplicator:
    ...
    def calculate_secure_hash(self, file_path: Path, db_conn: Optional[sqlite3.Connection] = None) -> Optional[str]:
        ...
        Args:
            ...
            db_conn: Optional existing database connection (for batch operations)
        """
        try:
            # Skip symlinks and special files
            if file_path.is_symlink():
            ...
            try:
                if db_conn:
                    ...
                    # Note: Caller is responsible for commit when passing db_conn
                else:
    ...
    def scan_directory(self, directory: Path, execute: bool = False, safety_threshold: float = 0.7) -> Dict:
        ...
        # Create shared connection for batch verification
        with sqlite3.connect(self.db_path) as conn:
            for group_idx, (quick_hash, file_list) in enumerate(potential_duplicates.items()):
                ...
                if (group_idx + 1) % 10 == 0 or (group_idx + 1) == total_groups:
                    ...
                    conn.commit()  # Periodically commit updates
    ...
    def clean_local_duplicates_of_gdrive(self, gdrive_dirs: List[Path], local_dirs: List[Path], execute: bool = False) -> Dict:
        ...
        # Create shared connection for cross-directory operations
        with sqlite3.connect(self.db_path) as conn:
