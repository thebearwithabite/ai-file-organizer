...
class BulletproofDeduplicator:
    ...
    def calculate_secure_hash(self, file_path: Path, conn: Optional[sqlite3.Connection] = None) -> Optional[str]:
        """
        ...
        Args:
            ...
            conn: Optional shared database connection for batch processing
        """
        try:
            # Skip symlinks and special files
            if file_path.is_symlink():
            ...
            try:
                if conn:
                    ...
                    # Use new connection with auto-commit via context manager
    ...
    def scan_directory(self, directory: Path, execute: bool = False, safety_threshold: float = 0.7) -> Dict:
        ...
        # Use a single connection for the verification phase to avoid N+1 connection overhead
        with sqlite3.connect(self.db_path) as conn:
            for group_idx, (quick_hash, file_list) in enumerate(potential_duplicates.items()):
                ...
                for i, file_path in enumerate(file_list):
                    secure_hash = self.calculate_secure_hash(file_path, conn=conn)

                    # Periodic commit to prevent huge transactions
                    if (i + 1) % 50 == 0:
                        conn.commit()
    ...
    def clean_local_duplicates_of_gdrive(self, gdrive_dirs: List[Path], local_dirs: List[Path], execute: bool = False) -> Dict:
        ...
        # Use shared connection for indexing loop
        with sqlite3.connect(self.db_path) as conn:
        ...
        # Use shared connection for scanning loop
        with sqlite3.connect(self.db_path) as conn:
